...
<p align="center">
  <b>🇷🇺 Русский</b> | <a href="ARCHITECTURE_en.md"><b>🇺🇸 English</b></a>
</p>
...
```
Пользователь
 ├── Windows → _Builder.bat   (PowerShell/Batch, интерактивное меню или CLI)
 └── Linux   → _Builder.sh    (Bash, интерактивное меню + параллельные сборки или CLI)
```

Оба файла — **паритетные обёртки**: одинаковые меню, одинаковая логика, разный синтаксис оболочки. Они также принимают **аргументы CLI** для неинтерактивного запуска — для скриптов и CI.

**CLI-команды (регистр не учитывается).** Необязательный первый аргумент задаёт режим: `ib` / `image` → Image Builder, `src` / `source` → Source Builder. Далее — команда и аргументы:
...
| `wizard`, `w` | — | Мастер создания нового профиля. |
| `clean`, `c` | `[тип] [id\|A]` | Очистка: тип 1–6 (Source) или 1–3 (Image); id или A = все. |
| `state`, `s` | — | Флаги профилей: где есть файлы/пакеты/сборки (F/P/S/M/H/X/OI/OS). |
...
- **_Builder.sh** — функция `load_lang()`: построчное чтение, замена плейсхолдеров через `${val//\{C_VAL\}/$C_VAL}` и т.д., `printf -v "$key"` для установки переменных.
- **_Builder.bat** — цикл `for /f` по файлу: `tokens=1,* delims==`, затем отложенная подстановка `!_v:{C_VAL}=%C_VAL%!` и т.д. (требуется `setlocal enabledelayedexpansion`). При отсутствии файла для выбранного языка используется `system/lang/en.env`.
...
### Система индикаторов профиля `[F P S M H X | OI OS]`

В главном меню отображается "хирургическая" панель ресурсов, позволяющая мгновенно оценить состав профиля:

| Индикатор | Описание |
|-----------|----------|
| **F** (Files) | Наличие файлового оверлея (`custom_files/%ID%/`). Здесь хранятся конфиги и файлы, попадающие в корень роутера. |
| **P** (Packages) | Наличие сторонних `.ipk` или `.apk` пакетов в `custom_packages/%ID%/` для импорта. |
| **S** (Source) | Наличие исходного кода пакетов (`src_packages/%ID%/`). |
| **M** (Manual Config) | Обнаружен активный файл `manual_config` — сохраненный результат сессии Menuconfig. |
| **H** (Hooks) | Обнаружен скрипт автоматизации `hooks.sh` в `custom_files/%ID%/`. |
| **X** (Patches) | Обнаружены патчи исходного кода (`custom_patches/%ID%/`). |
| **OI** | Наличие готовой прошивки от Image Builder (`firmware_output/imagebuilder/%ID%/`). |
| **OS** | Наличие готовой прошивки от Source Builder (`firmware_output/sourcebuilder/%ID%/`). |
...
nl_test/ nw_test/ ← тестовые распаковки дистрибутива; не источник истины
```
